...
describe Refactorings::RenameTemp do
  it 'changes all occurrences of the temp name, but only within the given scope' do
    ...
    CODE
    refactor(input, '2,2-2,6', 'billy', Raffle::Recorder.new).should match_code <<-CODE.undent
      def foo
        billy = 45
        june = billy
      end
  ...
  context 'with a block scoped temp' do
    it 'renames the temp inside the block' do
      ...
      CODE
      refactor(input, '2,2-2,6', 'bar', Raffle::Recorder.new).should match_code <<-CODE.undent
        def foo
          bar = 34
          [1].each do |number|
            puts number + bar
    ...
    it 'ignores block temps when the method temp is selected' do
      ...
      CODE
      refactor(input, '2,2-2,6', 'number', Raffle::Recorder.new).should match_code <<-CODE.undent
        def foo
          number = 34
          [1].each do |thing|
            puts thing
